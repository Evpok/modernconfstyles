...
\usepackage{csquotes}
\usepackage[backend=biber, style=lrec, bibencoding=utf8]{biblatex}
...
% The line below is not part of the template, but is used to denote differences from the official LREC style
\usepackage{changes}
...
bibliographical references\deleted{, except that, in order to make them appear in
a separate section, you have to use the \texttt{\textbackslash citelanguageresource} tag}.
Thus, a language resource should be cited as \parencite{speecon}.

\subsubsection{When Not Citing Any Language Resource}
\replaced{Just don't forget to remove the “Language resources” section.}{
...
the \texttt{.bib} file first, as a \texttt{@LanguageResource} (\added{case-insensitive}) item type, which
...
In order to generate a PDF file out of the LaTeX file herein\deleted{, when citing
language resources,} the following steps need to be performed:

\begin{itemize}
    \item Compile the \texttt{.tex} file once
    \item Invoke \replaced{\texttt{biber}}{\texttt{bibtex}} on the \replaced{corresponding}{eponymous} \replaced{\texttt{.bcf}}{\texttt{.aux}} file
    \item Compile the \texttt{.tex} file \deleted{twice}
\end{itemize}
